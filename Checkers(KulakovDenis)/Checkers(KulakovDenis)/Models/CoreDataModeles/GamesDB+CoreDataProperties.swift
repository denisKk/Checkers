...
extension GamesDB {
    ...
    func getMappedGame() -> GameHistory {
        return GameHistory(date: date,
                           timeLimit: TimeType(rawValue: Int(timeLimit)) ?? .unlimit, boardSize: BoardSize(rawValue: Int(boardSize)) ?? .size8x8,
                           players: players?.compactMap( { ($0 as! PlayersDataDB).getMappedPlayer() }) ?? []
        )
    }
}
