...
final class GameViewController: UIViewController {
    ...
    @IBOutlet private var topPlayerView: UserPanelView!
    @IBOutlet private var bottomPlayerView: UserPanelView!
    @IBOutlet private var chessboardView: UIView!
...
extension GameViewController {
    ...
    func setupActions() {
        ...
        game.showStep = { [weak self] fromTag, toTag in
            ...
            guard let fromView = self.chessboardView.viewWithTag(abs(fromTag)),
            ...
            if fromTag == toTag {
                if fromTag > 0 {
                    ...
                    UIView.transition(
                        ...
                        animations: {chessView.showIn(view: toView)},
                        completion: {_ in self.game.nextStep()})
                } else {
                    UIView.transition(
                        with: toView,
                        duration: 0.4,
                        options: .transitionFlipFromLeft,
                        animations: {chessView.removeFromSuperview()},
                        completion: { _ in self.game.nextStep()})
                }
            } else {
    ...
    func createChessBoard() {
        ...
        for row in 0..<BOARD_SIZE {
            for col in 0..<BOARD_SIZE {
                let frame = CGRect(
                    x: Double(row) * cellWidth,
                    y: chessboardView.frame.width - Double(col)*cellWidth - cellWidth,
                    width: cellWidth,
                    height: cellWidth)
                let cellView = UIView(frame: frame)
                cellView.tag = (col * BOARD_SIZE + row + 1)
                cellView.backgroundColor = isBlack ?
                BoardCellColor.black.rawWalue :
                BoardCellColor.white.rawWalue
                isBlack.toggle()
...
extension GameViewController {
               ...
               canStepArray.contains(view.tag) {
                ...
                UIView.animate(
                    ...
                    options: [.curveEaseInOut]) {
                        pannableView.center = CGPoint(
                            x: cellView.frame.width / 2,
                            y: cellView.frame.width / 2)
                    }
            }
        default:
            break
    ...
    func getCellViewBy(location: CGPoint) -> UIView? {
        for view in chessboardView.subviews
            where view.frame.contains(location) {
                return view
            }
        return nil
    }
    ...
    func showAgreeAlert(playerName: String) {
        let alert = UIAlertController(
            title: "A draw".localized + "?",
            message: "\(playerName) " + "offers a draw. Are you agree?".localized,
     ...
     func showFinishGameAlertWith(winnerName: String?) {
         ...
         let message = winnerName == nil ?
         "Game played to a draw".localized :
         "\(winnerName ?? "") " + "is winner".localized

         let alert = UIAlertController(
            title: "Finish game".localized,
            message: message + "!",
            preferredStyle: .alert)
...
extension GameViewController {
    ...
    @objc func stepAnimation() {
        ...
        if animationItem.value != 0 {
            ...
            UIView.transition(
                ...
                duration: 1,
                options: .transitionFlipFromLeft,
                animations: {chessView.showIn(view: cellView)})
            ...
            UIView.transition(
                ...
                duration: 1,
                options: .transitionFlipFromLeft,
                animations: { cellView.subviews.forEach {$0.removeFromSuperview()}})
        }
    ...
    func createAnimationArray() {
        ...
        for (index, value) in array.enumerated() {
            ...
            guard let oneView = chessboardView.viewWithTag(getTagByIndex(index)) else {return}
            let temp: Int
            ...
            if temp != value {
                animationsArray.append(AnimationItem(index: index, value: value))
            }
        }
    }
