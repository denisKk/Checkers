...
final class Settings: NSObject {
    ...
    var timeLimit: TimeType {
        get {
            TimeType(rawValue: UserDefaults.standard.integer(forKey: UserDefaultsKeys.timeLimit.rawValue)) ?? .unlimit
        }
        set {
    ...
    var boardSize: BoardSize {
        get {
            BoardSize(rawValue: UserDefaults.standard.integer(forKey: UserDefaultsKeys.boardSize.rawValue)) ?? .size8x8
        }
        set {
    ...
    var gradientColor: (UIColor, UIColor)? {
        get {
            guard
                ...
                let unarchivedFirst = try? NSKeyedUnarchiver.unarchivedObject(
        ...
        set {
            if let gColor = newValue {
                ...
                if let first: Data = try? NSKeyedArchiver.archivedData(
                    withRootObject: gColor.0,
                    requiringSecureCoding: false) as Data {
                    UserDefaults.standard.set(first, forKey: UserDefaultsKeys.startGradientColor.rawValue)
                }
                if let second: Data = try? NSKeyedArchiver.archivedData(
                    withRootObject: gColor.1,
                    requiringSecureCoding: false) as Data {
